...
# --- RUN INIT NOW THAT MODELS EXIST ---
safe_init()
...
def wheel():
    if 'uid' not in session: return redirect('/login')
    user=User.query.get(session['uid'])
    colors_hex=["#111827","#dc2626","#111827","#eab308","#111827","#16a34a","#111827","#2563eb","#111827","#ea580c","#111827","#db2777","#111827","#7c3aed","#111827","#0891b2"]
    svg=""
    for i in range(16):
        ...
        svg+=f'<path d="M100,100 L{x1:.1f},{y1:.1f} A95,95 0 0,1 {x2:.1f},{y2:.1f} Z" fill="{colors_hex[i]}" stroke="white" stroke-width="2"/>'
    return STYLE+f"""<div class=card><h2>🎡 COLOR WHEEL 💕</h2><div class='music-tag'>🎹 Music inside</div><p id='bal' style=color:#16a34a;font-weight:900>Balance: R{user.balance:.2f}</p><div style=position:relative><div style=position:absolute;top:-10px;left:50%;transform:translateX(-50%);font-size:36px;z-index:10>👇</div><div id='wheel' style='width:280px;height:280px;margin:10px auto;border-radius:50%;border:8px solid #facc15;overflow:hidden;transform:rotate(0deg);background:white'><svg viewBox="0 0 200 200" style=width:100%;height:100%>{svg}<circle cx="100" cy="100" r="16" fill="#0f172a" stroke="#facc15" stroke-width="3"/></svg></div></div><div style=text-align:left;font-weight:900;font-size:13px;margin:12px 0 6px 0>PICK COLOR:</div><div style=display:grid;grid-template-columns:repeat(4,1fr);gap:6px><button class='color-btn' style=background:#dc2626;color:white onclick="pickColor('red','RED',this)">🔴 RED</button><button class='color-btn' style=background:#eab308;color:black onclick="pickColor('yellow','YELLOW',this)">🟡 YEL</button><button class='color-btn' style=background:#16a34a;color:white onclick="pickColor('green','GREEN',this)">🟢 GREEN</button><button class='color-btn' style=background:#2563eb;color:white onclick="pickColor('blue','BLUE',this)">🔵 BLUE</button><button class='color-btn' style=background:#ea580c;color:white onclick="pickColor('orange','ORANGE',this)">🟠 ORANGE</button><button class='color-btn' style=background:#db2777;color:white onclick="pickColor('pink','PINK',this)">🩷 PINK</button><button class='color-btn' style=background:#7c3aed;color:white onclick="pickColor('purple','PURPLE',this)">🟣 PURPLE</button><button class='color-btn' style=background:#0891b2;color:white onclick="pickColor('cyan','CYAN',this)">🔷 CYAN</button></div><div id='picked' style=font-weight:900;margin:10px 0;min-height:20px;color:#9333ea>Pick color!</div><div id='wheel_res' style=font-weight:900;font-size:18px;min-height:24px></div><div id='wheel_win' style=font-weight:900;font-size:26px;min-height:32px></div><div style=display:grid;grid-template-columns:repeat(4,1fr);gap:6px;margin-top:6px><button class='btn btn-dark' onclick="setStake(1)">R1</button><button class='btn btn-dark' onclick="setStake(2)">R2</button><button class='btn btn-dark' onclick="setStake(5)">R5</button><button class='btn btn-dark' onclick="setStake(10)">R10</button></div><input id='wheel_stake' type='hidden' value='1'><div id='stakeShow' style=text-align:left;font-size:13px;font-weight:800;margin:6px 0>Stake: R1</div><button id='spinBtn' class='btn btn-purple' style=padding:16px;font-size:16px;opacity:0.5;pointer-events:none onclick="spinWheel()">PICK COLOR TO SPIN</button><button class='btn' style=background:#e2e8f0;color:#475569 onclick="location.href='/live'">BACK</button><script>
...
def wheel_spin():
    ...
    user.balance-=stake
    color_map={'red':(1,0.5),'yellow':(3,1),'green':(5,1.5),'blue':(7,2),'orange':(9,2.5),'pink':(11,3),'purple':(13,3.5),'cyan':(15,4.5)}
    labels=["0","RED","0","YELLOW","0","GREEN","0","BLUE","0","ORANGE","0","PINK","0","PURPLE","0","CYAN"]
    pick_idx = random.choices(range(16), weights=[12,4,12,5,12,4,12,3,12,3,12,2,12,2,12,1])[0]
    win=0
    if color in color_map:
        win_idx, win_mult = color_map[color]
        if pick_idx==win_idx: win=win_mult*stake; user.balance+=win
    else: user.balance+=stake
    db.session.commit()
    return {"win":round(win,2),"index":pick_idx,"landed_label":labels[pick_idx],"balance":round(user.balance,2)}
...
def redeem_voucher():
    ...
    if v and not v.is_used:
        ...
        return STYLE+f"<div class=card><h2>✅ R{v.amount} ADDED! (Mochaina)</h2><p>Balance: R{user.balance:.2f}</p><button class='btn btn-gold' onclick=\"location.href='/menu'\">MENU</button></div>"
    if v and v.is_used:
    ...
    return STYLE+f"<div class=card><h2>⏳ {vtype} Received!</h2><p>Code: {raw[:4]}****{raw[-4:] if len(raw)>4 else ''}</p><p>Claimed: R{amount}</p><p style=color:orange>Admin verifies in 5 mins. Keep voucher!</p><button class='btn btn-gold' onclick=\"location.href='/menu'\">MENU</button></div>"
...
def load_eft():
    ...
    return STYLE+f"<div class=card><h2>EFT R{amt}</h2><p>Ref: <b>{ref}</b></p><p>Bank: TymeBank 51088331090<br>Use Ref. Admin approves.</p><button onclick=\"location.href='/menu'\">Menu</button></div>"
...
def payfast_return(): return STYLE+"<div class=card><h2>✅ Payment Received! Wait confirmation.</h2><button class='btn btn-gold' onclick=\"location.href='/menu'\">Menu</button></div>"
