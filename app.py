    ...
    if request.method=='POST':
        try:
            ...
            if id_num and (len(id_num)!=13 or not id_num.isdigit()):
                return STYLE+"<div class=card><p style=color:red>❌ ID 13 digits</p><button onclick=\"location.href='/register'\">Back</button></div>"
            if p2 and p!=p2:
                return STYLE+"<div class=card><p style=color:red>❌ Passwords no match</p><button onclick=\"location.href='/register'\">Back</button></div>"
        ...
        except Exception as e: return STYLE+f"<div class=card><p>{e}</p></div>"
...
def live_games():
    if 'uid' not in session: return redirect('/login')
    user=User.query.get(session['uid'])
    return STYLE+f"""<div class=card><h2>🔴 LIVE GAMES</h2><p style=color:#16a34a;font-weight:900>Balance: R{user.balance:.2f}</p><div class='game-card' onclick="location.href='/coin'" style=border-left:6px solid #facc15><b>🪙 COIN FLIP</b></div><div class='game-card' onclick="location.href='/wheel'" style=border-left:6px solid #9333ea><b>🎡 WHEEL - BET WHILE SPIN - FIXED!</b></div><div class='game-card' onclick="location.href='/slots'" style=border-left:6px solid #2563eb><b>🎰 SLOTS</b></div><div class='game-card' onclick="location.href='/dice'" style=border-left:6px solid #16a34a><b>🎲 DICE</b></div><button class='btn' style=background:#e2e8f0;color:#475569;margin-top:16px onclick="location.href='/menu'">BACK</button></div>"""

# ================= FINAL FIXED WHEEL - CORRECT =================
            ...
            for b in bets:
                if b['round_id']==data['round_id']:
                    if b['color'].lower()==win_label.lower():
                        ...
                        if uid_str==str(session['uid']): my_win+=b['potential']
                else:
                    keep.append(b)
            if keep: remaining[uid_str]=keep
        db.session.commit()
        save_bets(remaining)
...
def wheel_auto_bet():
    ...
    data=get_wheel_round()
    time_left=data['end_time']-time.time()
    if time_left<WHEEL_CLOSE: return {"error":"STOP 1 sec! Showing win..."}
    color=request.args.get('color','red').lower()
    payouts={'red':2,'yellow':3,'green':2.5,'blue':2.5,'orange':4,'pink':4,'purple':5,'cyan':5}
    if color not in payouts: return {"error":"Invalid"}
    try: stake=int(float(request.args.get('stake',2)))
    except: stake=2
    user=User.query.get(session['uid'])
    if not user or stake>user.balance: return {"error":"No balance"}
    user.balance-=stake
    db.session.commit()
    bet_round=data['round_id']
    for_next=False
    if time_left <= SPIN_START:
    ...
    save_bets(all_bets)
    return {"balance":user.balance,"bet":bet,"for_next":for_next}
...
def buy():
    ...
    try:
        ...
        return STYLE+f"<div class=card><p>❌ Pick 4</p><button onclick=\"location.href='/play'\">Back</button></div>"
    if bet>user.balance: return STYLE+f"<div class=card>No bal R{user.balance:.2f}<br><button onclick=\"location.href='/play'\">Back</button></div>"
...
def load_funds():
    if 'uid' not in session: return redirect('/login')
    user=User.query.get(session['uid'])
    return STYLE+f"""<div class=card><h2>LOAD</h2><p>Balance: R{user.balance:.2f}</p><div class=tabs><div id='tab-voucher' class='tab active' onclick="showTab('voucher')">Vouchers</div><div id='tab-payfast' class='tab' onclick="showTab('payfast')">PayFast</div><div id='tab-eft' class='tab' onclick="showTab('eft')">EFT</div></div>
    <div id='voucher' class='tabcontent'><form method='post' action='/redeem_voucher'><select name='voucher_type'><option value='BLU'>Blu</option><option value='1VOUCHER'>1Voucher</option><option value='OTT'>OTT</option><option value='MOCHA'>Mochaina</option></select><input name='code' placeholder='PIN' required minlength=10><button class='btn btn-green'>REDEEM</button></form></div>
    <div id='payfast' class='tabcontent' style=display:none><form method='post' action='/payfast_pay'><input name='amount' type='number' value='50' min='10' max='5000' required><button class='btn btn-blue'>PAYFAST</button></form></div>
...
def redeem_voucher():
    ...
    if v and not v.is_used:
        ...
        return STYLE+f"<div class=card><h2>✅ R{v.amount} ADDED!</h2><p>Balance: R{user.balance:.2f}</p><button onclick=\"location.href='/menu'\">MENU</button></div>"
    if v and v.is_used:
        return STYLE+f"<div class=card><p>❌ Already used by {v.used_by}</p><button onclick=\"location.href='/load'\">Back</button></div>"
    if len(code)<8:
        return STYLE+f"<div class=card><p>❌ PIN min 8</p><button onclick=\"location.href='/load'\">Back</button></div>"
    amount=10
    ...
    if exist:
        return STYLE+f"<div class=card><p>⏳ Already submitted</p><button onclick=\"location.href='/menu'\">Menu</button></div>"
    p=Payment(user_id=user.id, username=user.username, amount=amount, ref=f"{vtype}-{code}", status="Pending", method=vtype)
    db.session.add(p); db.session.commit()
    return STYLE+f"<div class=card><h2>⏳ {vtype} Received R{amount}</h2><button onclick=\"location.href='/menu'\">MENU</button></div>"
...
def withdraw():
    ...
    if request.method=='POST':
        ...
        if amt<50 or amt>user.balance or len(acc)<6:
            return STYLE+f"<div class=card><p>Invalid - Min R50</p><button onclick=\"location.href='/withdraw'\">Back</button></div>"
