...
# ===== WHEEL FIXED - 12 COLORS DESIGN + GOOD ROTATION 2160 + 4.5s =====
@app.route('/wheel')
def wheel():
    if 'uid' not in session: return redirect('/login')
    user=User.query.get(session['uid'])
...
<div id='wheelWrap' style='width:100%;height:100%;border-radius:50%;border:8px solid #facc15;background:white;overflow:hidden;box-shadow:0 0 25px #facc1588'><svg viewBox="0 0 200 200" style=width:100%;height:100%>{svg}<circle cx="100" cy="100" r="24" fill="#0f172a" stroke="white" stroke-width="3"/><circle cx="100" cy="100" r="12" fill="#facc15"/></svg></div>
</div>
...
function setStake(v){{document.getElementById('stake').value=v; document.getElementById('stakeShow').innerText='Stake: R'+v+' on '+(sel||'-'); if(sel) enableBet();}}
function pickColor(c,m,el){{sel=c; selMult=m; document.querySelectorAll('.color-btn').forEach(b=>b.classList.remove('selected')); el.classList.add('selected'); document.getElementById('stakeShow').innerText='Stake: R'+document.getElementById('stake').value+' on '+c+' x'+m; enableBet(); try{{beep(600,0.1,0.10);}}catch(e){{}}}}
...
function doSpin(){{
  ...
  try{{soundWheelSpin();}}catch(e){{}}
  fetch('/wheel_spin_next?round='+roundId).then(r=>r.json()).then(d=>{{
    let center=d.index*30+15;
    let target=(360-center+360)%360;
    let finalRot=cr+2160+target;
    wrap.style.transition='transform 4.5s cubic-bezier(0.1,0.9,0.2,1)';
    ...
    setTimeout(function(){{
      ...
      try{{ if(d.win>0) soundWheelWin(); else soundWheelLose(); }}catch(e){{}}
...
def handle500(e): return STYLE+"<div class=card><p>Server busy, retry</p><button onclick=\"location.href='/menu'\">Menu</button></div>", 500
@app.errorhandler(404)
def handle404(e): return STYLE+"<div class=card><p>Page not found</p><button onclick=\"location.href='/menu'\">Menu</button></div>", 404
