...
def next_draw_str():
    try:
        nxt=get_next_draw_time(); total=int((nxt-datetime.now()).total_seconds())
...
def auto_draw_if_due():
    try:
        ...
        should=False
        if last:
            t12=now.replace(hour=12,minute=1,second=0,microsecond=0); t17=now.replace(hour=17,minute=1,second=0,microsecond=0)
            if last<t12<=now: should=True
            if last<t17<=now: should=True
        if should:
            ...
            won=False
            for tick in Ticket.query.all():
                u=User.query.get(tick.user_id)
                if not u: continue
...
def login():
    ...
    return STYLE+"""<div class=header><h1 style=color:#facc15>⭐ Mochaina Lotto 🇿🇦</h1></div><div style=background:linear-gradient(90deg,#facc15,#f97316);color:black;padding:12px;border-radius:12px;margin:10px;font-weight:900>🏆 JACKPOT R5,000,000</div><div class=card><h2>LOGIN</h2><form method='post'><input name='username' placeholder='Full name' required><input name='password' type='password' required><button class='btn btn-green'>Login</button></form><button class='btn btn-blue' onclick="location.href='/register'">Register</button></div>"""

@app.route('/register', methods=['GET','POST'])
def register():
    if request.method=='POST':
        full_name=request.form.get('full_name','').strip(); p=request.form.get('password',''); p2=request.form.get('confirm_password',''); id_num=request.form.get('id_number','')
        if id_num and (len(id_num)!=13 or not id_num.isdigit()):
            return STYLE+"<div class=card><p>❌ ID 13 digits</p><button onclick=\"location.href='/register'\">Back</button></div>"
        if p!=p2: return STYLE+"<div class=card><p>❌ Password no match</p><button onclick=\"location.href='/register'\">Back</button></div>"
        if User.query.filter_by(username=full_name).first(): return STYLE+"<div class=card><p>Exists</p></div>"
    ...
    return STYLE+"""<div class=header><h1 style=color:#facc15>⭐ Mochaina Lotto 🇿🇦</h1></div><div class=card><h2>Register</h2><form method='post'><input name='full_name' placeholder='Full name' required><input name='id_number' placeholder='ID 13 digits' required><input name='phone' placeholder='Phone' required><input name='password' type='password' placeholder='Password' required><input name='confirm_password' type='password' placeholder='Confirm' required><button class='btn btn-gold'>REGISTER</button></form><p><a href='/login'>Login</a></p></div>"""
...
def menu():
    ...
    return STYLE+f"""<div class=header><h1>⭐ MOCHAINA STAR ⭐</h1><p>Welcome, {session['uname']}</p></div><div class=card><div class=jackpot>R{get_jackpot():,.2f}</div><p style=color:green;font-weight:900>Balance: R{user.balance:.2f}</p><div><span id='liveTimer' class='timer-box'>{next_draw_str()}</span></div><script>startLiveCountdown({ts}, 'liveTimer')</script><button class='btn btn-green' onclick="location.href='/play'">PLAY LOTTO</button><button class='btn btn-dark' onclick="location.href='/my_tickets'">MY TICKETS</button><button class='btn btn-blue' onclick="location.href='/load'">LOAD FUNDS</button><button class='btn btn-orange' onclick="location.href='/withdraw'">WITHDRAW</button><button class='btn btn-purple' onclick="location.href='/results'">RESULTS</button><button class='btn' style=background:linear-gradient(90deg,#ef4444,#9333ea);color:white;padding:18px' onclick="location.href='/live'">🔴 LIVE GAMES</button><button class='btn' style=background:gray;color:white onclick="location.href='/logout'">LOGOUT</button></div>"""

@app.route('/live')
def live_games():
    if 'uid' not in session: return redirect('/login')
    user=User.query.get(session['uid'])
    return STYLE+f"""<div class=card><h2>🔴 LIVE GAMES</h2><p style=color:#16a34a;font-weight:900>Balance: R{user.balance:.2f}</p><div class='game-card' onclick="location.href='/coin'"><b>🪙 COIN FLIP</b></div><div class='game-card' onclick="location.href='/wheel'" style=border:3px solid #9333ea><b>🎡 WHEEL - FIXED! CHOOSE 1 WIN SHOWS!</b></div><div class='game-card' onclick="location.href='/slots'"><b>🎰 SLOTS</b></div><div class='game-card' onclick="location.href='/dice'"><b>🎲 DICE</b></div><button class='btn' style=background:#e2e8f0;color:#475569;margin-top:16px onclick="location.href='/menu'">BACK</button></div>"""

# ================= REBUILT WHEEL - CORRECT LOGIC =================
WHEEL_ROUND_FILE="wheel_round.json"
WHEEL_BETS_FILE="wheel_bets.json"
WHEEL_LAST_FILE="wheel_last.json"
WHEEL_TOTAL = 12
WHEEL_CLOSE = 1
SPIN_START = 8
...
def get_wheel_round():
    try:
        with open(WHEEL_ROUND_FILE,"r") as f:
            data=json.load(f)
...
def load_bets():
    try:
        with open(WHEEL_BETS_FILE,"r") as f: return json.load(f)
    except: return {}
def save_bets(b):
    with open(WHEEL_BETS_FILE,"w") as f: json.dump(b,f)
...
def wheel():
    ...
    return STYLE+f"""<div class=card><h2>🎡 WHEEL - REBUILT ✅</h2><div id='roundInfo' style=background:#0f172a;color:#facc15;padding:14px;border-radius:12px;font-weight:900;font-size:24px;border:2px solid #facc15>⏳ LOADING...</div><div id='betStatus' style=background:#dcfce7;color:#166534;padding:10px;border-radius:10px;margin:8px 0;font-weight:900>✅ BET NOW!</div><p id='bal' style=color:#16a34a;font-weight:900;font-size:18px;background:#dcfce7;padding:8px;border-radius:8px>Balance: R{user.balance:.2f}</p><div style=position:relative><div style=position:absolute;top:-10px;left:50%;transform:translateX(-50%);font-size:44px;z-index:10>👇</div><div id='wheel' style='width:300px;height:300px;margin:10px auto;border-radius:50%;border:10px solid #facc15;overflow:hidden;transform:rotate(0deg);background:white;box-shadow:0 0 30px #facc15aa'><svg viewBox="0 0 200 200" style=width:100%;height:100%>{svg}<circle cx="100" cy="100" r="22" fill="#0f172a" stroke="#facc15" stroke-width="4"/><circle cx="100" cy="100" r="10" fill="#facc15"/></svg></div></div><div id='lastWin' style=font-weight:900;font-size:22px;min-height:42px;background:#fffbeb;border:2px solid #facc15;padding:12px;border-radius:12px;margin:8px 0>Last: -</div><div style=text-align:left;font-weight:900;font-size:12px;margin:8px 0>CHOOSE ONE COLOR - WIN SHOWS WHEN STOP:</div><div style=display:grid;grid-template-columns:repeat(4,1fr);gap:8px><button class='color-btn' style=background:#dc2626;color:white onclick="pickColor('red','RED',this)">🔴 RED x2</button><button class='color-btn' style=background:#eab308;color:black onclick="pickColor('yellow','YELLOW',this)">🟡 YEL x3</button><button class='color-btn' style=background:#16a34a;color:white onclick="pickColor('green','GREEN',this)">🟢 GREEN x2.5</button><button class='color-btn' style=background:#2563eb;color:white onclick="pickColor('blue','BLUE',this)">🔵 BLUE x2.5</button><button class='color-btn' style=background:#ea580c;color:white onclick="pickColor('orange','ORANGE',this)">🟠 ORANGE x4</button><button class='color-btn' style=background:#db2777;color:white onclick="pickColor('pink','PINK',this)">🩷 PINK x4</button><button class='color-btn' style=background:#7c3aed;color:white onclick="pickColor('purple','PURPLE',this)">🟣 PURPLE x5</button><button class='color-btn' style=background:#0891b2;color:white onclick="pickColor('cyan','CYAN',this)">🔷 CYAN x5</button></div><div id='picked' style=font-weight:900;margin:10px 0;color:#9333ea>Pick ONE!</div><div id='myBets' style=text-align:left;font-size:13px;background:#f8fafc;padding:10px;border-radius:10px;min-height:24px;border:1px solid #e2e8f0>Your bet: none</div><div style=display:grid;grid-template-columns:repeat(4,1fr);gap:6px;margin-top:8px><button class='btn btn-dark' onclick="setStake(1)">R1</button><button class='btn btn-dark' onclick="setStake(2)">R2</button><button class='btn btn-dark' onclick="setStake(5)">R5</button><button class='btn btn-dark' onclick="setStake(10)">R10</button></div><input id='wheel_stake' type='hidden' value='2'><div id='stakeShow' style=text-align:left;font-size:13px;font-weight:800>Stake: R2</div><button id='betBtn' class='btn btn-purple' style=padding:18px;font-size:18px;opacity:0.5;pointer-events:none onclick="placeBet()">PICK COLOR</button><button class='btn' style=background:#e2e8f0;color:#475569 onclick="location.href='/live'">BACK</button><script>
let selectedColor=null;let cr=0;let currentRoundId=0;let canBet=true;let myCurrentBet=null;
function setStake(v){{document.getElementById('wheel_stake').value=v;document.getElementById('stakeShow').innerText='Stake: R'+v;if(selectedColor){{document.getElementById('betBtn').innerText='BET R'+v+' ON '+selectedColor.toUpperCase();}}}}
function pickColor(c,l,el){{if(!canBet){{alert('WAIT 1 sec showing win!');return;}}selectedColor=c;document.querySelectorAll('.color-btn').forEach(b=>b.classList.remove('selected'));el.classList.add('selected');document.getElementById('picked').innerText='Picked '+l;let btn=document.getElementById('betBtn');btn.style.opacity='1';btn.style.pointerEvents='auto';btn.innerText='BET R'+document.getElementById('wheel_stake').value+' ON '+l;}}
function placeBet(){{if(!selectedColor||!canBet)return;let stake=document.getElementById('wheel_stake').value;fetch('/wheel_auto_bet?color='+selectedColor+'&stake='+stake).then(r=>r.json()).then(d=>{{if(d.error){{alert(d.error);return;}}document.getElementById('bal').innerText='Balance: R'+d.balance.toFixed(2);myCurrentBet=d.bet;document.getElementById('myBets').innerHTML='THIS ROUND #'+d.bet.round_id+': '+d.bet.color.toUpperCase()+' R'+d.bet.stake+' → WIN R'+d.bet.potential+'<br><small>'+(d.is_next?'Bet for NEXT while spinning':'Bet for THIS round')+'</small>';}});}}
function updateRound(){{fetch('/wheel_auto_status').then(r=>r.json()).then(d=>{{currentRoundId=d.round_id;let left=Math.max(0,Math.floor(d.time_left));let spinning=left<=8 && left>1;document.getElementById('roundInfo').innerText=(spinning?'🎡 SPINNING: ':'⏳ BET: ')+left+'s | #'+d.round_id;if(d.last_label && d.last_label!='-'){{if(!d.just_finished){{document.getElementById('lastWin').innerText='Last: '+d.last_label;}}}}if(left<=8 && left>1 &&!window.spinningNow){{window.spinningNow=true;document.getElementById('betStatus').innerText='🔴 SPINNING 8s! You can BET FOR NEXT NOW!';document.getElementById('betStatus').style.background='#fef3c7';let w=document.getElementById('wheel');let center=d.winning_index*22.5+11.25;let target=(360-center+360)%360;let finalRot=cr+(6*360)+target+720;w.style.transition='transform 7.5s cubic-bezier(0.15,0.85,0.25,1)';w.style.transform='rotate('+finalRot+'deg)';setTimeout(()=>{{cr=finalRot%360;w.style.transition='none';w.style.transform='rotate('+cr+'deg)';window.spinningNow=false;}},7600);}}if(left<=1){{canBet=false;document.getElementById('betStatus').innerText='⏸️ STOP 1 SEC - SHOW WIN!';document.getElementById('betStatus').style.background='#fee2e2';document.getElementById('betBtn').innerText='STOP...';document.getElementById('betBtn').style.opacity='0.3';document.getElementById('betBtn').style.pointerEvents='none';}}else{{canBet=true;document.getElementById('betStatus').innerText='✅ '+(spinning?'BET FOR NEXT WHILE SPINNING '+left+'s':'BET NOW '+left+'s - Wheel will spin long');document.getElementById('betStatus').style.background='#dcfce7';if(selectedColor){{document.getElementById('betBtn').style.opacity='1';document.getElementById('betBtn').style.pointerEvents='auto';}}}}if(d.just_finished){{let text=d.my_win>0?'🎉 YOU WON R'+d.my_win+'! Landed '+d.last_label:'❌ YOU LOST! Landed '+d.last_label;document.getElementById('lastWin').innerText=text;document.getElementById('lastWin').style.background=d.my_win>0?'#dcfce7':'#fee2e2';document.getElementById('lastWin').style.color=d.my_win>0?'#166534':'#dc2626';document.getElementById('bal').innerText='Balance: R'+d.balance.toFixed(2);document.getElementById('myBets').innerHTML='Result: '+text+'<br>Pick new color for NEXT round!';document.querySelectorAll('.color-btn').forEach(b=>b.classList.remove('selected'));selectedColor=null;myCurrentBet=null;document.getElementById('picked').innerText='Pick for NEXT round!';document.getElementById('betBtn').innerText='PICK COLOR';document.getElementById('betBtn').style.opacity='0.5';if(d.my_win>0)soundWheelWin();else soundWheelLose();}}}});}}
setInterval(updateRound,500);updateRound();
</script></div>"""
...
def wheel_auto_status():
    ...
    last_label="-"
    try:
        with open(WHEEL_LAST_FILE,"r") as f: last_label=json.load(f).get('label','-')
    except: pass

    if time_left<=0:
        win_label=data['labels'][data['winning_index']]
        all_bets=load_bets()
        # PAY ONLY CURRENT ROUND
        to_keep={}
        for uid_str, bets in all_bets.items():
            keep=[]
            for b in bets:
                if b['round_id']==data['round_id']:
                    if b['color'].lower()==win_label.lower():
                        u=User.query.get(int(uid_str))
                        if u: u.balance+=b['potential']
                        if uid_str==str(session['uid']):
                            my_win+=b['potential']
                else:
                    keep.append(b)
            if keep:
                to_keep[uid_str]=keep
        save_bets(to_keep)
        db.session.commit()
        with open(WHEEL_LAST_FILE,"w") as f: json.dump({"label":win_label},f)
        last_label=win_label
        try: os.remove(WHEEL_ROUND_FILE)
        except: pass
        data=get_wheel_round()
        time_left=data['end_time']-time.time()
        just_finished=True
        session['last_win']=my_win
        session.modified=True
    else:
        my_win=session.get('last_win',0)

    user=User.query.get(session['uid'])
    return {"round_id":data['round_id'],"time_left":time_left,"winning_index":data['winning_index'],"last_label":last_label,"just_finished":just_finished,"my_win":my_win if just_finished else 0,"balance":user.balance if user else 0}
...
def wheel_auto_bet():
    ...
    time_left=data['end_time']-time.time()
    if time_left<1:
        return {"error":"STOP 1 sec - showing win!"}
    ...
    user=User.query.get(session['uid'])
    if not user or stake>user.balance:
        return {"error":f"No balance R{user.balance:.2f}" if user else "login"}

    user.balance-=stake
    db.session.commit()

    # CORRECT LOGIC: if spinning, bet goes to NEXT round, else THIS round
    bet_round=data['round_id']
    is_next=False
    if time_left <= 8:
        bet_round=data['round_id']+1
        is_next=True

    bet={"round_id":bet_round,"color":color,"stake":stake,"potential":round(stake*payouts[color],2)}
    all_bets=load_bets()
    uid_str=str(session['uid'])
    if uid_str not in all_bets: all_bets[uid_str]=[]
    # One bet per round_id
    all_bets[uid_str]=[b for b in all_bets[uid_str] if b['round_id']!=bet_round]
    all_bets[uid_str].append(bet)
    save_bets(all_bets)

    return {"balance":user.balance,"bet":bet,"is_next":is_next}
...
def slots():
    if 'uid' not in session: return redirect('/login')
    user=User.query.get(session['uid'])
    return STYLE+f"""<div class=card><h2>🎰 SLOTS</h2><p id='slotBal' style=color:#16a34a;font-weight:900>Balance: R{user.balance:.2f}</p><div style=background:#0f172a;padding:18px;border-radius:20px><div style=display:flex;justify-content:center;gap:10px><div class='reel-box'><div id='r1' class='reel'>🍒</div></div><div class='reel-box'><div id='r2' class='reel'>🍋</div></div><div class='reel-box'><div id='r3' class='reel'>🔔</div></div></div></div><div id='slot_res'></div><div id='slot_win'></div><div style=display:grid;grid-template-columns:repeat(4,1fr);gap:6px><button class='btn btn-dark' onclick="setSlotStake(1)">R1</button><button class='btn btn-dark' onclick="setSlotStake(2)">R2</button><button class='btn btn-dark' onclick="setSlotStake(5)">R5</button><button class='btn btn-dark' onclick="setSlotStake(10)">R10</button></div><input id='slot_stake' type='hidden' value='1'><div id='slotStakeShow'>Stake: R1</div><button id='spinBtn' class='btn btn-blue' onclick="spinSlot()">SPIN</button><button class='btn' style=background:#e2e8f0;color:#475569 onclick="location.href='/live'">BACK</button></div><script>let slotSymbols=["🍒","🍋","🔔","7️⃣","⭐","💎"];function setSlotStake(v){{document.getElementById('slot_stake').value=v;document.getElementById('slotStakeShow').innerText='Stake: R'+v;}}function spinSlot(){{let stake=document.getElementById('slot_stake').value;let btn=document.getElementById('spinBtn');if(btn.disabled)return;btn.disabled=true;btn.innerText='SPINNING...';let r1=document.getElementById('r1'),r2=document.getElementById('r2'),r3=document.getElementById('r3');r1.classList.add('spinning');r2.classList.add('spinning');r3.classList.add('spinning');let inter1=setInterval(()=>r1.innerText=slotSymbols[Math.floor(Math.random()*slotSymbols.length)],60);let inter2=setInterval(()=>r2.innerText=slotSymbols[Math.floor(Math.random()*slotSymbols.length)],70);let inter3=setInterval(()=>r3.innerText=slotSymbols[Math.floor(Math.random()*slotSymbols.length)],80);fetch('/slots_spin?stake='+stake).then(r=>r.json()).then(d=>{{setTimeout(()=>{{clearInterval(inter1);r1.classList.remove('spinning');r1.innerText=d.reels[0];}},1000);setTimeout(()=>{{clearInterval(inter2);r2.classList.remove('spinning');r2.innerText=d.reels[1];}},1900);setTimeout(()=>{{clearInterval(inter3);r3.classList.remove('spinning');r3.innerText=d.reels[2];document.getElementById('slot_res').innerText=d.msg;document.getElementById('slotBal').innerText='Balance: R'+d.balance.toFixed(2);btn.disabled=false;btn.innerText='SPIN AGAIN';}},2800);}});}}</script>"""
...
def slots_spin():
    ...
    if stake>user.balance: return {"reels":["❌","❌","❌"],"msg":"No bal","win":0,"balance":user.balance}
    ...
    if random.random()<0.7:
        ...
        reels=[random.choice(symbols) for _ in range(3)]
        if reels[0]==reels[1]==reels[2]: win=stake*10; msg=f"JACKPOT R{win}!"
...
def dice_page():
    if 'uid' not in session: return redirect('/login')
    user=User.query.get(session['uid'])
    return STYLE+f"""<div class=card><h2>🎲 DICE</h2><p>Balance: R{user.balance:.2f}</p><div class="table"><div id="num" style=position:absolute;top:30px;left:0;right:0;font-size:56px;font-weight:900;color:#facc15>?</div><div id="d1" class="dice"></div><div id="d2" class="dice dice2"></div><div id="hand" class="hand">✋🏾</div></div><div id="diceRes"></div><input id='dice_stake' type='hidden' value='2'><div style=display:grid;grid-template-columns:repeat(4,1fr);gap:6px><button class='btn btn-dark' onclick="setStake(1)">R1</button><button class='btn btn-dark' onclick="setStake(2)">R2</button><button class='btn btn-dark' onclick="setStake(5)">R5</button><button class='btn btn-dark' onclick="setStake(10)">R10</button></div><div id='stakeShow'>Stake: R2</div><div style=display:grid;gap:8px><button id='low' class='btn btn-green' onclick="playDice('low')">LOW 0-39</button><button id='high' class='btn btn-blue' onclick="playDice('high')">HIGH 61-100</button></div><button class='btn' style=background:#e2e8f0;color:#475569;margin-top:8px onclick="location.href='/live'">BACK</button></div><script>function setStake(v){{document.getElementById('dice_stake').value=v;document.getElementById('stakeShow').innerText='Stake: R'+v;}}setDice('d1',2);setDice('d2',5);function playDice(choice){{let stake=document.getElementById('dice_stake').value;let d1=document.getElementById('d1'),d2=document.getElementById('d2'),hand=document.getElementById('hand');document.getElementById('low').disabled=true;document.getElementById('high').disabled=true;document.getElementById('diceRes').innerText='Shaking...';d1.className='dice';d2.className='dice dice2';hand.className='hand';void d1.offsetWidth;d1.classList.add('throw1');d2.classList.add('throw2');hand.classList.add('handThrow');d1.style.opacity=1;d2.style.opacity=1;let t=0,inter=setInterval(function(){{setDice('d1',Math.floor(Math.random()*6)+1);setDice('d2',Math.floor(Math.random()*6)+1);document.getElementById('num').innerText=Math.floor(Math.random()*101);if(++t>16)clearInterval(inter);}},70);setTimeout(function(){{fetch('/dice_roll?stake='+stake+'&choice='+choice).then(r=>r.json()).then(d=>{{clearInterval(inter);setDice('d1',d.d1);setDice('d2',d.d2);document.getElementById('num').innerText=d.roll;document.getElementById('diceRes').innerText=d.msg;setTimeout(()=>location.reload(),1600);}});}},1500);}}</script>"""
@app.route('/dice_roll')
def dice_roll():
    if 'uid' not in session: return redirect('/login')
    user=User.query.get(session['uid'])
...
def coin_page():
    if 'uid' not in session: return redirect('/login')
    user=User.query.get(session['uid'])
    return STYLE+f"""<div class=card><h2>🪙 COIN</h2><p>Balance: R{user.balance:.2f}</p><div class="coinBox"><div class="coin" id="coin">R</div></div><div id="coinRes"></div><div style=display:grid;grid-template-columns:repeat(4,1fr);gap:6px><button class='btn btn-dark' onclick="setStake(1)">R1</button><button class='btn btn-dark' onclick="setStake(2)">R2</button><button class='btn btn-dark' onclick="setStake(5)">R5</button><button class='btn btn-dark' onclick="setStake(10)">R10</button></div><input id='coin_stake' type='hidden' value='2'><div id='stakeShow'>Stake: R2</div><div style=display:grid;grid-template-columns:1fr 1fr;gap:8px><button id='hBtn' class='btn btn-green' onclick="playCoin('heads')">HEADS</button><button id='tBtn' class='btn btn-blue' onclick="playCoin('tails')">TAILS</button></div><button class='btn' style=background:#e2e8f0;color:#475569;margin-top:8px onclick="location.href='/live'">BACK</button></div><script>function setStake(v){{document.getElementById('coin_stake').value=v;document.getElementById('stakeShow').innerText='Stake: R'+v;}}let busy=false;function playCoin(choice){{if(busy)return;busy=true;let stake=document.getElementById('coin_stake').value;let coin=document.getElementById('coin');let res=document.getElementById('coinRes');let speed=55,rot=0;coin.innerText='?';res.innerText='Flipping...';document.getElementById('hBtn').disabled=true;document.getElementById('tBtn').disabled=true;let timer=setInterval(function(){{rot+=speed;coin.style.transform='rotateY('+rot+'deg) scale(1.2)';if(rot<800)speed=55;else if(rot<1600)speed=35;else if(rot<2400)speed=18;else if(rot<3000)speed=7;else{{clearInterval(timer);fetch('/coin_flip?stake='+stake+'&choice='+choice).then(r=>r.json()).then(d=>{{let finalRot=Math.ceil(rot/360)*360;coin.style.transform='rotateY('+finalRot+'deg) scale(1)';setTimeout(function(){{coin.innerText=d.result=='heads'?'👑':'🦅';res.innerText=d.win>0?'WON R'+d.win:'LOST '+d.result;busy=false;document.getElementById('hBtn').disabled=false;document.getElementById('tBtn').disabled=false;setTimeout(()=>location.reload(),1500);}},400);}});}}}},16);}}</script>"""
@app.route('/coin_flip')
def coin_flip():
    if 'uid' not in session: return redirect('/login')
    user=User.query.get(session['uid'])
    ...
    if stake>user.balance: return {"error":"No bal","result":"none","win":0}
...
def buy():
    ...
    try:
        ...
        if len(nums)!=4 or len(set(nums))!=4: raise ValueError
    except: return STYLE+"<div class=card><p>❌ Pick 4</p></div>"
    if bet>user.balance: return STYLE+f"<div class=card>No bal R{user.balance:.2f}</div>"
...
def load_funds():
    if 'uid' not in session: return redirect('/login')
    user=User.query.get(session['uid'])
    return STYLE+f"""<div class=card><h2>LOAD</h2><p>Balance: R{user.balance:.2f}</p><div class=tabs><div id='tab-voucher' class='tab active' onclick="showTab('voucher')">Vouchers</div><div id='tab-payfast' class='tab' onclick="showTab('payfast')">PayFast</div><div id='tab-eft' class='tab' onclick="showTab('eft')">EFT</div></div><div id='voucher' class='tabcontent'><form method='post' action='/redeem_voucher'><select name='voucher_type'><option>Blu</option></select><input name='code' placeholder='PIN' required><button class='btn btn-green'>REDEEM</button></form></div><div id='payfast' class='tabcontent' style=display:none><form method='post' action='/payfast_pay'><input name='amount' type='number' value='50' min='10' max='5000' required><button class='btn btn-blue'>PAYFAST</button></form></div><div id='eft' class='tabcontent' style=display:none><form method='post' action='/load_eft'><button name='amount' value='50' class='btn btn-dark'>R50</button><button name='amount' value='100' class='btn btn-dark'>R100</button></form></div><br><button class='btn' style=background:gray;color:white onclick="location.href='/menu'">BACK</button></div>"""
...
def redeem_voucher():
    ...
    if v and not v.is_used:
        u=User.query.get(session['uid']); u.balance+=v.amount; v.is_used=True; v.used_by=u.username; db.session.commit()
        return STYLE+f"<div class=card><h2>✅ R{v.amount} ADDED!</h2><button onclick=\"location.href='/menu'\">MENU</button></div>"
    if v and v.is_used: return STYLE+"<div class=card><p>Already used</p></div>"
    amount=10
    ...
    if Payment.query.filter_by(ref=f"{vtype}-{code}", status="Pending").first(): return STYLE+"<div class=card><p>Already submitted</p></div>"
    p=Payment(user_id=u.id, username=u.username, amount=amount, ref=f"{vtype}-{code}", status="Pending", method=vtype); db.session.add(p); db.session.commit()
    return STYLE+f"<div class=card><h2>⏳ {vtype} R{amount} Received</h2><button onclick=\"location.href='/menu'\">MENU</button></div>"
...
def load_eft():
    ...
    return STYLE+f"<div class=card><h2>EFT R{amt}</h2><p>Ref: {ref}</p><button onclick=\"location.href='/menu'\">Menu</button></div>"
...
def payfast_pay():
    ...
    pf_str="&".join([f"{k}={urllib.parse.quote_plus(str(v).strip())}" for k,v in data.items()])
...
def payfast_return(): return STYLE+"<div class=card><h2>✅ Payment Received!</h2><button onclick=\"location.href='/menu'\">Menu</button></div>"
...
def payfast_notify():
    try:
        ...
        if uid and amount>0:
            ...
            if u: u.balance+=amount; p=Payment(user_id=uid, username=u.username, amount=int(amount), ref=request.form.get('m_payment_id','PF'), status="Completed", method="PayFast"); db.session.add(p); db.session.commit()
    except: pass
    return "OK",200
...
def withdraw():
    ...
    if request.method=='POST':
        ...
        if amt<50 or amt>user.balance or len(acc)<6: return STYLE+"<div class=card><p>Invalid</p></div>"
...
def admin():
    ...
    return STYLE+f"""<div class=card><h2>ADMIN - R{get_jackpot():,.2f}</h2><h3>Pending</h3><div>{ph}</div><h3>Vouchers</h3><div>{vh}</div><br><form method='post' action='/admin_gen_voucher?key=mochaina123'><input name='code' placeholder='CODE' required><input name='amount' type='number' required><button class='btn btn-blue'>CREATE</button></form><br><button class='btn btn-red' onclick="location.href='/admin_draw/now?key=mochaina123'">DRAW NOW</button><br><br><button class='btn' style=background:gray onclick="location.href='/menu'">Menu</button></div>"""
...
def admin_gen_voucher():
    ...
    if amount>0 and not Voucher.query.filter_by(code=code).first(): db.session.add(Voucher(code=code, amount=amount)); db.session.commit()
