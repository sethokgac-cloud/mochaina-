...
def live_games():
    if 'uid' not in session: return redirect('/login')
    user=User.query.get(session['uid'])
    return STYLE+f"""<div class=card><h2>🔴 LIVE GAMES 💕</h2><div class='music-tag'>🎹 Romantic music inside</div><p style=color:#16a34a;font-weight:900>Balance: R{user.balance:.2f}</p><div class='game-card' onclick="location.href='/coin'" style=border-left:6px solid #facc15><b>🪙 COIN FLIP</b><span style=float:right>▶️</span></div><div class='game-card' onclick="location.href='/wheel'" style=border-left:6px solid #9333ea><b>🎡 AUTO WHEEL - LONG SPIN!</b><span style=float:right>▶️</span></div><div class='game-card' onclick="location.href='/slots'" style=border-left:6px solid #2563eb><b>🎰 SLOTS ONE BY ONE</b><span style=float:right>▶️</span></div><div class='game-card' onclick="location.href='/dice'" style=border-left:6px solid #16a34a><b>🎲 DICE THROW</b><span style=float:right>▶️</span></div><button class='btn' style=background:#e2e8f0;color:#475569;margin-top:16px onclick="location.href='/menu'">BACK TO MENU</button></div>"""

# ================= LONG SPIN AUTO WHEEL =================
WHEEL_ROUND_FILE="wheel_round.json"
WHEEL_TOTAL = 18 # 10 sec BET + 8 sec LONG SPIN = 18 sec total
WHEEL_CLOSE = 8 # last 8 sec = LONG SPIN (fast->slow->stop)
...
function pickColor(c,l,el){{if(!canBet){{return;}}selectedColor=c;document.querySelectorAll('.color-btn').forEach(b=>b.classList.remove('selected'));el.classList.add('selected');document.getElementById('picked').innerText='Picked '+l;let btn=document.getElementById('betBtn');btn.style.opacity='1';btn.style.pointerEvents='auto';btn.innerText='BET R'+document.getElementById('wheel_stake').value+' ON '+l;beep(600,0.1,0.10);}}
function placeBet(){{if(!selectedColor||!canBet)return;let stake=document.getElementById('wheel_stake').value;fetch('/wheel_auto_bet?color='+selectedColor+'&stake='+stake+'&round='+currentRoundId).then(r=>r.json()).then(d=>{{if(d.error){{alert(d.error);return;}}document.getElementById('bal').innerText='Balance: R'+d.balance.toFixed(2);myBetsForRound.push(d.bet);let h='Bets:<br>';myBetsForRound.forEach(b=>{{h+=b.color.toUpperCase()+' R'+b.stake+'<br>';}});document.getElementById('myBets').innerHTML=h;}});}}
function updateRound(){{fetch('/wheel_auto_status').then(r=>r.json()).then(d=>{{currentRoundId=d.round_id;let left=Math.max(0,Math.floor(d.time_left));document.getElementById('roundInfo').innerText='⏳ '+(d.status=='BETTING'?'BET: '+left+'s':'SPIN: '+left+'s')+' | #'+d.round_id;if(d.status=='SPINNING'){{canBet=false;document.getElementById('betStatus').innerText='🔴 LONG SPIN! '+left+'s - FAST->SLOW!';document.getElementById('betStatus').style.background='#fee2e2';document.getElementById('betStatus').style.color='#dc2626';document.getElementById('betBtn').innerText='SPINNING LONG...';document.getElementById('betBtn').style.opacity='0.3';document.getElementById('betBtn').style.pointerEvents='none';if(!window.spinningNow){{window.spinningNow=true;let w=document.getElementById('wheel');let center=d.winning_index*22.5+11.25;let target=(360-center+360)%360;let spins=6;let finalRot=cr+(spins*360)+target+720;w.style.transition='transform 7.5s cubic-bezier(0.15,0.85,0.25,1)';w.style.transform='rotate('+finalRot+'deg)';setTimeout(()=>{{cr=finalRot%360;w.style.transition='none';w.style.transform='rotate('+cr+'deg)';window.spinningNow=false;}},7800);}} }} else {{canBet=true;document.getElementById('betStatus').innerText='✅ BET NOW - '+left+'s left';document.getElementById('betStatus').style.background='#dcfce7';document.getElementById('betStatus').style.color='#166534';}} if(d.just_finished){{if(d.my_win>0){{document.getElementById('lastWin').innerText='WON R'+d.my_win+'! '+d.last_label;document.getElementById('lastWin').style.color='#16a34a';soundWheelWin();}} else {{document.getElementById('lastWin').innerText='Landed '+d.last_label+(myBetsForRound.length>0?' - Lost':'');document.getElementById('lastWin').style.color='#ef4444';if(myBetsForRound.length>0) soundWheelLose();}} document.getElementById('bal').innerText='Balance: R'+d.balance.toFixed(2);myBetsForRound=[];document.getElementById('myBets').innerHTML='Your bets: none';}} }});}}
...
def wheel_auto_bet():
    ...
    data=get_wheel_round()
    time_left=data['end_time']-time.time()
    if time_left<WHEEL_CLOSE: return {"error":"Spinning! Wait next round 1 sec..."}
