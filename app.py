...
def live_games():
    if 'uid' not in session: return redirect('/login')
    user=User.query.get(session['uid'])
    return STYLE+f"""<div class=card><h2>🔴 LIVE GAMES 💕</h2><div class='music-tag'>🎹 Romantic music + low sounds inside</div><p style=color:#16a34a;font-weight:900>Balance: R{user.balance:.2f}</p><div class='game-card' onclick="location.href='/coin'" style=border-left:6px solid #facc15><b>🪙 COIN FLIP</b><span style=float:right>▶️</span></div><div class='game-card' onclick="location.href='/wheel'" style=border-left:6px solid #9333ea><b>🎡 WHEEL AUTO (NEW!)</b><span style=float:right>▶️</span></div><div class='game-card' onclick="location.href='/slots'" style=border-left:6px solid #2563eb><b>🎰 SLOTS ONE BY ONE</b><span style=float:right>▶️</span></div><div class='game-card' onclick="location.href='/dice'" style=border-left:6px solid #16a34a><b>🎲 DICE THROW</b><span style=float:right>▶️</span></div><button class='btn' style=background:#e2e8f0;color:#475569;margin-top:16px onclick="location.href='/menu'">BACK TO MENU</button></div>"""

# ===== NEW SPECIAL AUTO WHEEL - ROLL 3-4 SEC + BET FOR NEXT ROW DURING ROLL =====
@app.route('/wheel')
def wheel():
    if 'uid' not in session: return redirect('/login')
    user=User.query.get(session['uid'])
    colors=[("RED","#dc2626"),("YELLOW","#eab308"),("GREEN","#16a34a"),("BLUE","#2563eb"),("ORANGE","#ea580c"),("PINK","#db2777"),("PURPLE","#7c3aed"),("CYAN","#0891b2")]
    svg="".join([f'<path d="M100,100 L{100+95*math.cos(math.radians(i*45-90)):.1f},{100+95*math.sin(math.radians(i*45-90)):.1f} A95,95 0 0,1 {100+95*math.cos(math.radians((i+1)*45-90)):.1f},{100+95*math.sin(math.radians((i+1)*45-90)):.1f} Z" fill="{hexc}" stroke="white" stroke-width="2"/>' for i,(_,hexc) in enumerate(colors)])
    return STYLE+f"""<div class=card><h2>🎡 AUTO WHEEL LIVE</h2>
    <div style=background:#0f172a;color:white;padding:10px;border-radius:12px;font-weight:900;display:flex;justify-content:space-between>
    <span>ROUND <span id='roundId' style=color:#facc15>1</span></span><span id='phase' style=color:#4ade80>BET OPEN</span><span id='countdown' style=color:#facc15>10</span>s</div>
    <p id='bal' style=color:#16a34a;font-weight:900>Balance: R{user.balance:.2f}</p>
    <div style=position:relative><div style=position:absolute;top:-8px;left:50%;transform:translateX(-50%);font-size:30px;z-index:10>👇</div>
    <div id='wheel' style='width:280px;height:280px;margin:10px auto;border-radius:50%;border:8px solid #facc15;overflow:hidden;transform:rotate(0deg);background:white'><svg viewBox="0 0 200 200" style=width:100%;height:100%>{svg}<circle cx="100" cy="100" r="16" fill="#0f172a" stroke="#facc15" stroke-width="3"/></svg></div></div>
    <div id='lastWin' style=font-weight:900;min-height:26px;font-size:18px>Last: -</div>
    <div id='myBetBox' style=background:#fef3c7;padding:8px;border-radius:10px;font-weight:800;margin:6px 0>Next bet: -</div>
    <div style=display:grid;grid-template-columns:repeat(4,1fr);gap:6px>
    <button class='color-btn' style=background:#dc2626;color:white onclick="pick('RED',this)">RED x3</button>
    <button class='color-btn' style=background:#eab308;color:black onclick="pick('YELLOW',this)">YEL x3</button>
    <button class='color-btn' style=background:#16a34a;color:white onclick="pick('GREEN',this)">GREEN x3</button>
    <button class='color-btn' style=background:#2563eb;color:white onclick="pick('BLUE',this)">BLUE x3</button>
    <button class='color-btn' style=background:#ea580c;color:white onclick="pick('ORANGE',this)">ORG x4</button>
    <button class='color-btn' style=background:#db2777;color:white onclick="pick('PINK',this)">PINK x5</button>
    <button class='color-btn' style=background:#7c3aed;color:white onclick="pick('PURPLE',this)">PURP x8</button>
    <button class='color-btn' style=background:#0891b2;color:white onclick="pick('CYAN',this)">CYAN x12</button>
    </div>
    ...
    <input id='stake' type='hidden' value='5'><div id='stakeShow' style=text-align:left;font-size:13px;font-weight:800;margin:4px 0>Stake: R5</div>
    <button id='betBtn' class='btn btn-gold' style=padding:16px;opacity:0.5;pointer-events:none onclick="placeBet()">PICK COLOR</button>
    <div id='winBox' style=font-weight:900;font-size:22px;min-height:28px;margin-top:6px></div>
    <button class='btn' style=background:#e2e8f0;color:#475569;margin-top:8px onclick="location.href='/live'">BACK</button>
    <script>
    let sel=null, cr=0, roundId=1, timeLeft=10, phase='betting', spinning=false, idle=null;
    ...
    function pick(c,el){{sel=c; document.querySelectorAll('.color-btn').forEach(b=>b.classList.remove('selected')); el.classList.add('selected'); document.getElementById('stakeShow').innerText='Stake: R'+document.getElementById('stake').value+' on '+c; enableBet();}}
    function enableBet(){{let b=document.getElementById('betBtn'); b.style.opacity='1'; b.style.pointerEvents='auto'; if(!spinning) b.innerText='LOCK BET FOR ROUND '+roundId+' — R'+document.getElementById('stake').value+' ON '+sel; else b.innerText='BET FOR NEXT ROUND '+(roundId+1)+' — WHILE ROLLING!';}}
    function placeBet(){{if(!sel)return; let s=document.getElementById('stake').value; fetch('/wheel_place_next?color='+sel+'&stake='+s).then(r=>r.json()).then(d=>{{if(d.error){{alert(d.error);return;}} document.getElementById('myBetBox').innerText='Locked for Round '+(spinning?roundId+1:roundId)+': R'+s+' on '+sel; document.getElementById('bal').innerText='Balance: R'+d.balance.toFixed(2); document.getElementById('betBtn').style.opacity='0.5'; document.getElementById('betBtn').style.pointerEvents='none'; document.getElementById('betBtn').innerText='BET LOCKED ✓ — PICK NEXT'; sel=null; document.querySelectorAll('.color-btn').forEach(b=>b.classList.remove('selected'));}});}}
    function startIdle(){{idle=setInterval(()=>{{if(!spinning){{cr+=0.5; document.getElementById('wheel').style.transform='rotate('+cr+'deg)';}}}},30);}}
    function loop(){{
        setInterval(()=>{{
            if(!spinning){{
    ...
    function doSpin(){{
        spinning=true; clearInterval(idle); document.getElementById('phase').innerText='ROLLING 3s... BET NEXT OPEN!'; document.getElementById('phase').style.color='#facc15';
        fetch('/wheel_spin_next?round='+roundId).then(r=>r.json()).then(d=>{{
            let center=d.index*45+22.5; let target=(360-center+360)%360; let finalRot=cr+1080+target;
            let w=document.getElementById('wheel'); w.style.transition='transform 3.5s cubic-bezier(0.1,0.9,0.2,1)'; w.style.transform='rotate('+finalRot+'deg)';
            roundId++; document.getElementById('roundId').innerText=roundId; timeLeft=8;
            document.getElementById('myBetBox').innerText='Next bet: - (bet NOW while rolling!)';
            setTimeout(()=>{{
                cr=finalRot%360; w.style.transition='none'; w.style.transform='rotate('+cr+'deg)';
                document.getElementById('lastWin').innerText='Round '+(roundId-1)+' landed '+d.landed;
                ...
                spinning=false; document.getElementById('phase').innerText='BET OPEN'; document.getElementById('phase').style.color='#4ade80';
                startIdle();
            }},3600);
        }});
    }}
...
def wheel_place_next():
    ...
    if color not in ["RED","YELLOW","GREEN","BLUE","ORANGE","PINK","PURPLE","CYAN"]: return {"error":"Invalid color"}
    ...
    user.balance-=stake
    wheel_next_bets[uid]={"color":color,"stake":stake}
    db.session.commit()
...
def wheel_spin_next():
    if 'uid' not in session: return {"error":"login"}
    uid=session['uid']; user=User.query.get(uid)
    colors=["RED","YELLOW","GREEN","BLUE","ORANGE","PINK","PURPLE","CYAN"]
    mult={"RED":3,"YELLOW":3,"GREEN":3,"BLUE":3,"ORANGE":4,"PINK":5,"PURPLE":8,"CYAN":12}
...
def redeem_voucher():
    ...
    return STYLE+f"<div class=card><h2>⏳ {vtype} Received!</h2><p>Code: {raw[:4]}****</p><p>Claimed: R{amount}</p><button class='btn btn-gold' onclick=\"location.href='/menu'\">MENU</button></div>"
...
def admin():
    ...
    return STYLE+f"""<div class=card><h2>ADMIN - R{get_jackpot():,.2f}</h2><p>Users: {total_users} | Bal: R{total_bal:.2f}</p><h3>Pending</h3><div>{ph}</div><h3>Unused Vouchers</h3><div>{vh}</div><br><form method='post' action='/admin_gen_voucher?key=mochaina123'><input name='code' placeholder='MOCHA-100-XXX' required><input name='amount' type='number' required><button class='btn btn-blue'>CREATE VOUCHER</button></form><br><button class='btn btn-red' onclick="location.href='/admin_draw/now?key=mochaina123'">DO DRAW NOW</button><br><br><button class='btn' style=background:gray onclick="location.href='/menu'">Menu</button></div>"""
