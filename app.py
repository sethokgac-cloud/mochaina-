...
def live_games():
    if 'uid' not in session: return redirect('/login')
    user=User.query.get(session['uid'])
    return STYLE+f"""<div class=card><h2>🔴 LIVE GAMES 💕</h2><div class='music-tag'>🎹 Romantic music inside</div><p style=color:#16a34a;font-weight:900>Balance: R{user.balance:.2f}</p><div class='game-card' onclick="location.href='/coin'" style=border-left:6px solid #facc15><b>🪙 COIN FLIP</b><span style=float:right>▶️</span></div><div class='game-card' onclick="location.href='/wheel'" style=border-left:6px solid #9333ea><b>🎡 AUTO WHEEL - 2 MIN LIVE!</b><span style=float:right>▶️</span></div><div class='game-card' onclick="location.href='/slots'" style=border-left:6px solid #2563eb><b>🎰 SLOTS ONE BY ONE</b><span style=float:right>▶️</span></div><div class='game-card' onclick="location.href='/dice'" style=border-left:6px solid #16a34a><b>🎲 DICE THROW</b><span style=float:right>▶️</span></div><button class='btn' style=background:#e2e8f0;color:#475569;margin-top:16px onclick="location.href='/menu'">BACK TO MENU</button></div>"""

# ================= AUTO WHEEL 2 MIN + 4 SEC =================
...
def get_wheel_round():
    try:
        ...
        # FIXED: 2 minutes = 120 sec + 4 sec = 124 sec total
        data={"round_id":int(time.time()),"end_time":time.time()+124,"winning_index":random.randint(0,15),"labels":labels}
...
def wheel_auto_status():
    ...
    # FIXED: only 4 sec spinning, rest 2 min betting
    status="BETTING" if time_left>4 else "SPINNING"
    just_finished=False
    if time_left<=0:
        labels=data['labels']
...
def wheel_auto_bet():
    ...
    data=get_wheel_round()
    time_left=data['end_time']-time.time()
    if time_left<4: return {"error":"Betting closed! Wait 4 sec..."}
